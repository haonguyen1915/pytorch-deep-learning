...
from CV.models.residual_net import ResidualBlock, ResNet
import torch
...
def evaluation(data_generator):
    model = torch.load(f"{PRJ_DIR}/weights/ResNet.pt")
    model.eval()
...
def process_inputs(inputs, labels):
    ...
    inputs_padded = torch.zeros((inputs.shape[0], 32*32))
    inputs_padded[:, :inputs.shape[1]] = inputs[:, :inputs.shape[1]]
    inputs_padded = inputs_padded.view(inputs.shape[0], 1, 32, 32)
...
if __name__ == "__main__":
    ...
    net = ResNet(ResidualBlock, [2, 2, 2], num_classes=cal_generator.num_output)
